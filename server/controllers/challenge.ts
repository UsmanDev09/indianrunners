...
import CategoryModel, {
  Category as ChallengeCategoryInterface,
} from "../models/challengeCategory";
import LeaderboardModel from "../models/leaderboard";
import { Constants } from "../utility/constants";
import logger from "../config/logger";
import UserModel, { User as UserInterface } from "../models/user";

export const createChallenge: RequestHandler<
  unknown,
  unknown,
  ChallengeInterface,
  unknown
  ...
  try {
    ...
    if (type === "open" && lowerLimit === undefined)
      throw createHttpError(
        ...
        Constants.openChallengesShouldHaveLowerLimit
      );

    if (type === "open" && (upperLimit || fixedLimit))
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.openChallengesWrongOptions
      );

    if (
      type === "fixed" &&
      (upperLimit === undefined || fixedLimit === undefined)
    )
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.fixedChallengesShouldHaveUpperLimitAndFixedLimit
      );

    if (type === "fixed" && lowerLimit)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.fixedChallengesWrongOptions
      );

    if (activity === "single" && categories?.length !== 1)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.singleCategoryShouldBeProvided
      );

    if (activity === "multiple" && categories?.length < 2)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.multipleCategoriesShouldBeProvided
      );

    if (knockout && knockoutType === undefined)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.knockoutTypeShouldBeProvided
      );

    if (!knockout && (cutOffDays || cutOffHours))
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.challengesWrongOptions
      );

    if (knockoutType === "hourly" && cutOffHours === undefined)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.hourlyKnockoutChallengesMustHaveCutOffHours
      );

    if (knockoutType === "daily" && cutOffDays === undefined)
      throw createHttpError(
        StatusCodes.BAD_REQUEST,
        Constants.dailyKnockoutChallengesMustHaveCutOffDays
      );
...
export const addCertificateToChallenge: RequestHandler<
  unknown,
  unknown,
  { challengeId: number; certificateUrl: string; designState: object },
  unknown
> = async (req, res, next) => {
  try {
...
export const getCertificatesByChallenge: RequestHandler<
  { challengeId: number },
  unknown,
  unknown,
  unknown
...
// logic not completed
export const updateChallenge: RequestHandler<
  unknown,
  unknown,
  ChallengeInterface,
  unknown
...
export const deleteChallenge: RequestHandler<
  { id: number },
  unknown,
  ChallengeInterface,
  unknown
...
export const getAllChallenges: RequestHandler<
  unknown,
  unknown,
  ChallengeInterface,
  any
> = async (req, res, next) => {
  try {
    ...
    if (activity) filters.activity = new RegExp(activity, "i");

    if (knockout) filters.knockout = knockout;

    if (knockoutType) filters.knockoutType = new RegExp(knockoutType, "i");
    ...
    if (minPrice && !isNaN(minPrice)) filters.price = { $gte: minPrice };

    if (maxPrice && !isNaN(maxPrice)) {
      filters.price = { $lte: maxPrice };
    }

    if (name === "asc" || name === "desc") sort.name = name;
...
export const getUsersCertificateStatus: RequestHandler<
  { id: number },
  unknown,
  ChallengeInterface,
  unknown
...
export const assignCertificateToAUser: RequestHandler<
  unknown,
  unknown,
  ChallengeInterface,
  unknown
> = async (req, res, next) => {
  try {
    ...
    await ChallengeModel.findOneAndUpdate(
      { _id: challengeId, userDetails: { user: userId } },
      { certificatesSent: true }
    );

    const user = await UserModel.findByIdAndUpdate(userId, {
      $push: { certicates: challenge.certificate },
    });
...
export const getChallengeById: RequestHandler<
  { id: number },
  unknown,
  ChallengeInterface,
  ChallengeInterface
