...
const Leaderboard = ({ leaderboards } : Leaderboard[]) => {

    return (
        <div className="container mx-auto flex md:flex-nowrap flex-wrap">
            <ProfileSideBar />
            {leaderboards ? leaderboards.map((leaderboard: Leaderboard, index: number ) => {
                return (
                    <div key={index} className="flex flex-wrap">    
                        ...
                        <div className="bg-white rounded-b-lg p-4">
                            <h2 className="text-xl font-semibold mb-4 underline underline-offset-4 text-center">{leaderboard.challenge?.name} </h2>

                            <ul>
                                <li className="flex items-center justify-between py-2">
                                    <div className="flex">
                                        <span className="text-gray-600 mr-2 flex items-center ">{leaderboard.userDetails?.rank}</span>
                                        <Image src="/chelsea.png" width={32} height={32} alt="Club Logo"/>
                                    </div>
                                    <div className="flex items-center">
                                    <div>
                                        <p className="font-semibold">{leaderboard.userDetails?.user?.firstName}</p>
                                        <p className="text-sm text-gray-500">{leaderboard.userDetails?.user?.club}</p>
                                    </div>
                                    </div>
                                    <span className="text-gray-600">{leaderboard.userDetails?.distance}</span>
                                </li>
                            </ul>
