  ...
  return (
    <div className="container mx-auto pt-20">
      {notifications && notifications.map((notification, index) => {
        ...
        const timeDifference = currentTime - date; // This will give the difference in milliseconds
        ...
        <div key={index} className="flex bg-white shadow-lg mb-2"> 
          <div className="w-full rounded-lg sm:p-4 mt-2">
              <h5 className="mb-2 text-3xl font-bold text-gray-900">{notification.type}</h5>
              <p className="mb-5 text-base text-gray-600 sm:text-md">{notification.message}</p>
              <p className="text-xs text-pink">
                {days > 0 && `${days} ${days === 1 ? 'day' : 'days'}`} 
                {hours > 0 && ` ${hours} ${hours === 1 ? 'hour' : 'hours'}`}
                {minutes > 0 && ` ${minutes} ${minutes === 1 ? 'minute' : 'minutes'}`}
                {seconds > 0 && ` ${seconds} ${seconds === 1 ? 'seconds' : 'seconds'}`} ago
              </p>
          </div>
          <button type="button" onClick={onDeleteNotification} className="items-center justify-center flex rounded-lg p-1.5 hover:bg-gray-400 h-8 w-8">
