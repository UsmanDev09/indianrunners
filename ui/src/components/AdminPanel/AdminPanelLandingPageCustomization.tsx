...
const AdminPanelLandingPageCustomization = ({
  landingpage,
}: {
  landingpage: any;
}) => {
  const [
    openCreateCustomizedlandingPageDrawer,
    setOpenCreateCustomizedLandingPageDrawer,
  ...
  const updateImageUrlInDB = async (image_url: string) => {
    try {
      if (image_url)
        await axios.put(`${process.env.SERVER_DOMAIN}/api/landingpage`, {
          image: image_url,
        });
      ...
      <div className="flex flex-col ">
        <div className="overflow-x-auto">
          <div className="inline-block min-w-full align-middle">
            <h3 className="font-medium">Main Section:</h3>
            {loading ? (
              <Loader />
            ) : (
              ...
              >
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    SVG, PNG, JPG or GIF (1920x1080px)
                  </p>
                </div>
                <input
                  id="dropzone-file"
                  type="file"
                  className="hidden"
                  onChange={uploadLandingPageBackgroundToCloudinary}
                />
              </label>
            </div>
                ...
                <thead className="bg-gray-100 dark:bg-gray-700">
                  <tr>
                    ...
                    <button
                      ...
                      type="button"
                      className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-gray-200 dark:text-white rounded-lg bg-gray-700 hover:bg-gray-800 focus:ring-4 mt-2 focus:ring-primary-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                    >
                      <svg
                        className="w-4 h-4 mr-2 text-gray-100 group-hover:text-gray-900 dark:text-gray-400"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                ...
                <tbody className="bg-white divide-y divide-gray-300 dark:bg-dark-green dark:divide-gray-700">
                  {sections &&
                    sections.map((section: any, index: number) => {
                      return (
                        <>
                          <tr className="hover:bg-gray-100 dark:hover:bg-transparent">
                            <td
                              key={index}
                            ...
                            <td className="p-4 text-base font-bold text-gray-900 whitespace-nowrap dark:text-white">
                              {section?.type}
                            </td>
                            <td>
                              <div className="flex flex-wrap gap-2">
                                {section.type === "product" ? (
                                  section.products?.map(
                                    (inventory: Inventory, index: number) => {
                                      return (
                                        <td key={index}>
                                          <div>
                                            <span className="bg-gray-200 text-gray-800 text-lg font-bold me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                                              {inventory?.name}
                                            </span>
                                          </div>
                                        </td>
                                  ...
                                  <td className="p-4 font-bold text-gray-500 dark:text-gray-200">
                                    None
                                  </td>
                                )}
                              </div>
                            ...
                            <td>
                              <div className="flex flex-col">
                                {section.type === "challenge" ? (
                                  section.challenges.map(
                                    (challenge: Challenge, index: number) => {
                                      return (
                                        <td key={index}>
                                          <div className="px-5 pb-5">
                                            <span className="bg-gray-200 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-200">
                                              {challenge.name}
                                            </span>
                                          </div>
                                        </td>
                            ...
                            <td className="p-4 space-x-2 whitespace-nowrap">
                              ...
                              >
                                <svg
                                  className="w-4 h-4 mr-2 text-gray-100 hover:text-gray-900 dark:text-gray-200"
                                  fill="currentColor"
                                  viewBox="0 0 20 20"
                                  xmlns="http://www.w3.org/2000/svg"
                                >
                              ...
                              <button
                                ...
                                aria-controls="drawer-delete-product-default"
                                data-drawer-placement="right"
                                className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-gray-200 dark:text-white bg-gray-700 rounded-lg hover:bg-gray-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900"
                              >
                                <svg
                                  className="w-4 h-4 mr-2 text-gray-100 hover:text-gray-900 dark:text-gray-200"
                                  fill="currentColor"
