...
const DeleteInventory = ( {setInventory, Inventory, setOpenDeleteProductDrawer, openDeleteProductDrawer } : { setInventory: (products: Inventory[]) => void, Inventory: Inventory, setOpenDeleteProductDrawer : (action: boolean) => void, openDeleteProductDrawer: boolean }) => {
    const [ formData, setFormData ] = useState({_id: Inventory._id})
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
          const response = await fetch(`${process.env.SERVER_DOMAIN}/api/inventory/${formData._id}`, {
              method: 'DELETE',
    ...
    return( 
        <div id="drawer-delete-product-default" className={`fixed right-0  mt-10 ${openDeleteProductDrawer ? 'block' : 'hidden'} z-40 w-full h-screen max-w-xs p-4 overflow-y-auto bg-white dark:bg-gray-800`}  aria-labelledby="drawer-label" aria-hidden="true">
            ...
            <svg className="w-10 h-10 mt-8 mb-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
